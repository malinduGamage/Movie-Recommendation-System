...
/* ── OPTIMIZATION 3: findusers() eliminated ──────────────────────────────
 * The original findusers() function has been merged into get_utility_matrix()
 * to avoid reading the same CSV file twice. See utility_matrix.c for the
...
void recommender(int uid){
	printf("starting recommendation process...\n");

	/* ORIGINAL CODE:
	 * int No_of_users = findusers();  // Optimization 3: eliminated
	 */
	...
	/* ── OPTIMIZATION 3: Merged findusers + get_utility_matrix ───────
	 * BEFORE: Called findusers() to get No_of_users, then malloc'd the
	 * matrix, then called get_utility_matrix() to fill it — reading the
	 * same CSV file twice.
	 *
	 * AFTER: Single call to get_utility_matrix() which finds No_of_users
	 * internally (first pass), allocates the matrix, and fills it
	 * (second pass using rewind). Returns No_of_users.
	 *
	 * ORIGINAL CODE:
	 * int No_of_users = findusers();
	 * double *utility_matrix = (double *)malloc(sizeof(double) * No_of_users * No_of_movies);
	 * get_utility_matrix(utility_matrix,"Dataset/ratings_learn.csv",No_of_movies, No_of_users, userid);
	 * ──────────────────────────────────────────────────────────────────── */
	...
	/* ── OPTIMIZATION 2: Memory copy instead of file re-read ─────────
	 * BEFORE: Re-read the entire ratings_learn.csv to extract one user's
	 * ratings, even though the data was already in the utility matrix.
	 *
	 * ORIGINAL CODE:
	...
	// printf("User ratings for movies:\n");
	// for (i = 0; i < 5; i++) {
	// 	if (newuser[i] > 0) {
	// 		printf("Movie %d: %.1lf\n", i + 1, newuser[i]);
	// 	}
	...
	double *normalizednewuser = calloc(No_of_movies, sizeof(double));

	normalize(newuser,normalizednewuser,No_of_movies); //normalizing the users ratings for similarity calculations
	printf("Normalized new user ratings:\n");
	for(i = 0; i < 5; i++) {
			printf("Movie %d: %.3lf\n", i + 1, normalizednewuser[i]);
	}
	double *similarity = malloc(sizeof(double) * No_of_users);

	/* ── OPTIMIZATION 1: calc_similarity uses direct pointer ─────────
	 * See pearsons.c for the original code. The function now points
	 * directly to matrix rows instead of malloc+copy+free per user.
	 * ──────────────────────────────────────────────────────────────────── */
